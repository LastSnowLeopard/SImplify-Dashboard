...
const EmailPreview = ({data, onClick, content, setData}) => {
    ...
    return (
<div className={`py-[27px] px-[22px] flex flex-col gap-5 lg:w-[330px] lg:h-[calc(100vh+200px)] rounded-r-lg max-h-[90vh] overflow-y-auto relative ${styles.emailPreviewContainer}`}
        style={{ background: 'linear-gradient(0deg, #222222, #222222), #E0DCF9' }}>
            ...
            <div className="flex flex-col gap-3.5">
                <Text15 color="white" title={`Email Preview (To: ` + content?.receiverEmail + ")"} />
                {content !== undefined && 
                    <div className="bg-[#696FEC] flex items-center justify-center text-center w-9 h-9 rounded-full">
                        <span className="text-sm text-white font-inter font-semibold uppercase">
                            {content?.senderEmail.split(" ")[0]?.slice(0, 2).toUpperCase() || "NA"}
                        </span>
                    </div>
                }
                <div className="flex flex-col gap-[18px]">
                    <div className="flex flex-col gap-[5px]">
                        <Text15 color="white" title={content?.senderEmail ? `${content?.senderEmail.split(" ")[0]} (${content?.senderEmail.split("<")[1]?.split(">")[0] || content?.senderEmail || "n/a"})` : "n/a"} />
                        <Text15 color="primary" title={content?.subject} />
                    </div>
                    <div className="h-full max-h-[400px] overflow-y-auto flex flex-col gap-[18px] no-scrollbar">
    {content !== undefined &&
        <span className={`text-xs leading-[15px] lg:text-xxs lg:leading-4 2xl:text-mini 2xl:leading-[18px] font-inter font-semibold text-primary`}>
            {content?.text}
        </span>
    }
</div>
            ...
            {isMobile && 
                <div className="z-50">
                    <button className="text-black bg-white text-sm rounded-lg py-2 px-3" onClick={onClick}>
                        Back
                    </button>
                </div>
            }
