...
export const CreateButton = ({title, onClick, children, size}) => {
    return (
        <button onClick={onClick} className={`border-none outline-none send-bg gap-2 py-2 flex items-center  xl:px-3 px-4 rounded-lg ${size==="big"?"w-full lg:w-[140px] justify-between":size==="large"?"w-full lg:w-[170px] justify-between":"justify-center"}`}>
...
export const AddButton = ({title, onClick, children}) => {
    return (
        <button onClick={onClick} className='border-none outline-none add-bg flex gap-2 py-2 px-4 rounded-lg'>
            <span className='text-white font-inter font-semibold leading-4 text-xxs'>
                    {title}
            </span>
            {children}
...
export const GrayButton =({title, onClick, size})=>{
    return (
        ...
        ${size==="big"?"px-[34px]":size==="small"?"px-7":size==="xs"?"px-4":"px-6"}
         outline-none`}>
