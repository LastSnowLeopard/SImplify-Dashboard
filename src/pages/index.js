        ...
        await fetch(`${api_url}/text`, {
          ...
          .then((data) => {
            ...
            if (aiResponse !== "") {
              ...
              setUserMessageList((prev) =>
                prev.map((message) =>
                  message.message === '...' ? { ...message, message: aiResponse } : message
                )
              );
            }
          })
      ...
      <div className='flex flex-col gap-4 w-full pt-5 lg:pt-6 pb-7 px-[26px] lg:pr-0 lg:pl-8'>
        <div className='flex items-center justify-between w-full'>
          ...
          <div className='rounded-full hidden lg:flex items-center gap-6 justify-between px-2 py-[3.5px] bg-mediumseagreen'>
            <span className='font-inter font-semibold text-forestgreen text-xs leading-[15px]'>Online</span>
        ...
        <div className='flex flex-col gap-5 flex-grow'>
              ...
              >
                <div className='grid grid-cols-1 px-7 py-10' key={index}>
                  <div className='col-span-1 flex flex-col gap-30 mr-auto'>
                    {message.type === 'ai' && (
                      <div className={`flex items-end gap-2 ${isLoading ? 'text-white' : ''}`}>
                  ...
                  <div className='col-span-1 flex flex-col gap-30 ml-auto mt-10'>
                    {message.type === 'user' && <div className='flex items-end gap-2'>
          ...
          <div className='flex items-center border-px border-[#0000000D] send-input-bg rounded-lg' style={{ zIndex: 3 }}>
  <div className='py-3.5 px-3 hidden lg:flex items-center justify-center text-plum text-sm leading-4 font-inter font-medium bg-purple-400 bg-opacity-20' style={{ zIndex: 4 }}>
    CHAT
  </div>
  <div className='flex items-center justify-between w-full pr-2.5 py-1.5 relative' 
       style={{ minHeight: '40px', zIndex: 1 }}> {/* Lower z-index so that CHAT and Send buttons can overlay */}
    ...
    <textarea
      ...
      className="absolute inset-0 w-full h-full pr-12 resize-none flex items-center" // Added flex and items-center
      style={{ zIndex: 0, backgroundColor: '#202020', color: 'white' }} 
      rows="1"
    ></textarea>

    <div className="absolute right-0 top-0 bottom-0 flex items-center" style={{ zIndex: 2 }}> {/* Higher z-index than textarea */}
