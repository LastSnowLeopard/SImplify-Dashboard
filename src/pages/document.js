...
const DocumentsPage = () => {
    ...
    const handleFilter = async () => {
        ...
        if(value!==""){
            ...
            await fetch(`${api_url}/documents/types/${formattedValue}/${phoneNumber}`, options)
            .then(res => res.json())
            .then(res => {
                console.log(res);
            ...
            <div className='flex flex-col gap-4 w-full pt-6 pb-7 px-[26px] xl:pr-0 xl:pl-8 '>
                <div className="flex flex-col gap-3 2xl:flex-row 2xl:items-center justify-between w-full">
                    ...
                    <div className='hidden xl:flex items-center gap-2'>
                        <SearchInput
                            className={"relative bg-[#191919] flex items-center rounded-lg h-9 2xl:w-[387px]"}
                            value={value}
                            onChange={onChange}
                            onClick={handleSearchClick}
                            placeholder={"Search Documents"} />
                        <GrayButton size="xs" title="Filter by Type" onClick={handleFilter} />
                        <CreateButton title="Create" onClick={() => setIsCreate(true)} size="big">
                            <AddSvg color="#FFFFFF" />
                        </CreateButton>
                    </div>
                </div>
                <div className="flex flex-col xl:hidden gap-3.5">
                    <div className="flex items-center gap-3">
                        <CreateButton title="Create" onClick={() => setIsCreate(true)} size="small">
                            <AddSvg color="#FFFFFF" />
                        </CreateButton>
                ...
                <div className='rounded-xl xl:mt-6 w-full h-full'>
                    <div className="grid grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-2.5 lg:gap-5 overscroll overflow-y-auto max-h-[453px] 2xl:max-h-[500px]">
                        {
                            data.map((item, index) => (
                                <div
                                    ...
                                    key={index}
                                    className={`col-span-2 lg:col-span-3 xl:col-span-1 text-[#6A6A6C] flex flex-col gap-3.5 docu-bg bg-gray-800 py-2 px-2.5 lg:py-3 lg:px-4 rounded-3xs shadow-5xl h-fit ${selected === index ? "border-px border-solid border-primary border-opacity-50" : ""}`}>
                                    <div className="flex items-center gap-2">
                                        <Image alt="file" src={item.type==="docs"?fileSvg:pptImg} />
                                        <Text15 color="white" title={item.topic} />
                                    </div>

                                    <div className="flex items-center gap-2 text-gray-800">
                                        <ActionButton background="gray-800" onClick={() => handleDelete(index)}>
                                            <TrashSvg />
                                        </ActionButton>
                                    </div>
                                </div>
