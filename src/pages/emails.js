...
const EmailsPage = () => {
    ...
    const time12Hour = debriefingData.time ? convertTo12HourFormat(debriefingData.time) : '';
    return (
        <div className="bg-[#141414] flex flex-row gap-6 2xl:gap-12 w-full rounded-lg">
           {!showEmailPreview &&  <div className='flex flex-col gap-4 w-full pt-6 pb-7 px-[26px] xl:pr-0 xl:pl-8 '>
                <div className="flex flex-col gap-3 2xl:flex-row 2xl:items-center justify-between w-full">
                    ...
                    <div className='flex items-center gap-2'>
                        <SearchInput
                            className={"relative hidden bg-[#191919] xl:flex items-center rounded-lg h-9 2xl:w-[387px]"}
                            value={value}
                            onChange={onChange}
                            onClick={handleSearch}
                            placeholder={"Search Email"} />
                        <GrayButton size="xs" title="Sort" onClick={handleFilter} />
                        <CreateButton title="Setup" onClick={() => setIsCreate(true)} size="big">
                            <AddSvg color="#FFFFFF" />
                        </CreateButton>
                    </div>
                </div>
                    ...
                    <div className="flex flex-col gap-3 2xl:flex-row 2xl:items-center justify-between w-full">
                        <div className='flex items-start gap-2.5'>
                            ...
                            <div className='flex flex-col gap-1'>
                                ...
                                <Text13 color="primary"
                                    title="Overview your latest emails" />
                            </div>
                        </div>
                        <div className='flex items-center gap-2 w-full md:w-auto'>
                            <SearchInput
                                className={"relative bg-[#191919] flex items-center rounded-lg h-9 w-full 2xl:w-[387px]"}
                                value={value}
                                setValue={setValue}
                                placeholder={"Search Email"} />
                        </div>
                        ...
                        !loading ?
                            <>
                                {Array.isArray(data) && data.length > 0 ? data.map((item, index) => (
                                    <div className="grid grid-cols-4 items-center py-4 gap-5" key={index}>
                                        <div className="col-span-4 lg:col-span-1 flex items-center gap-2.5">
                                            <div className="bg-[#696FEC] flex items-center justify-center text-center w-9 h-9 rounded-full">
                                                <span className="text-sm text-white font-inter font-semibold">
                                                    {item.senderEmail.split(" ")[0]?.slice(0, 2).toUpperCase() || "NA"}
                                                </span>
                                            </div>
                                            <div className="flex flex-col gap-[5px] flex-1">
                                                <Text15 color="white" title={item.senderEmail.split(" ")[0] || "n/a"} />
                                                <span className={`text-xs w-[229px] leading-[15px] lg:text-xxs lg:leading-4 2xl:text-mini 2xl:leading-[18px] font-inter font-semibold text-primary block truncate `}>
                                                    {item.subject}
                                                </span>
                                            </div>
                                        </div>
                                        ...
                                        <div className="col-span-4 lg:col-span-1 flex items-center justify-between">
                                            <div className="flex flex-col gap-[5px]">
                                                <Text15 color="white" title={new Date(Number(item.timestamp._seconds) * 1000).toDateString()} />
                                                <Text15 color="primary" title={new Date(Number(item.timestamp._seconds) * 1000).toLocaleTimeString()} />
                                            </div>
                                            <button onClick={()=>handleRead(item)} className="flex items-center gap-[11px] bg-white rounded-lg py-2 px-4">
           ...
           {isMobile? showEmailPreview && <div className="w-full">
                <EmailPreview content={readContent} data={data} setData={setData} onClick={onClick} />
